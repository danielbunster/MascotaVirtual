...
public class Inventario{
    ...
    public void buscaID(int id,Mascota e){  // Método encargado de buscar el objeto seleccionado en el menu, y removiendo si se acaba.
        int i=0;
        int remove =-1;  // ojo con la interpretacion del problema
        for(Item item : inventario){ // Creo haber leído que existe una condición para los juguetes, que se usan y se eliminan, no entendí muy bien eso.
            i = i + 1;
            if(id == (item.id)){
                e.darItem(item);
